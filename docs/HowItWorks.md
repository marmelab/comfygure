...
From a security standpoint, if the remote server is owned, the attacker can only access the encrypted data. Since the server never has access to the decryption key, the attacker can't decrypt the configuration.

By default, the server is comfy.marmelab.com (run by marmelab), but it can be a server that you host yourself ([this repository contains the server code](https://github.com/marmelab/comfygure)) and the [related documentation](./AdvancedUsage.html#host-your-own-comfy-server).
