...
export const create = λ(async event => {
    ...
    return addConfiguration(projectId, environmentName, configName, tagName, entries, format);
});
...
export const history = λ(async event => {
    ...
    await checkAuthorizationOr403(parseAuthorizationToken(event), projectId, 'read');
    const all =
        event.queryStringParameters && Object.keys(event.queryStringParameters).includes('all');
